...
static int drm_allocate_dumb(struct drm_buffer *buf)
{
	...
	prime_handle.handle = buf->handle;
	ret = drmIoctl(drm_dev.fd, DRM_IOCTL_PRIME_HANDLE_TO_FD, &prime_handle);
	if (ret < 0) {
		struct drm_mode_destroy_dumb dreq;
		memset(&dreq, 0, sizeof(dreq));
		dreq.handle = buf->handle;
		err("DRM_IOCTL_PRIME_HANDLE_TO_FD error");
		drmIoctl(drm_dev.fd, DRM_IOCTL_MODE_DESTROY_DUMB, &dreq);
		return -1;
	}
	buf->dmabuf_fd = prime_handle.fd;
